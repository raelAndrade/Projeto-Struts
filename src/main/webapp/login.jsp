	...
	if (msie > 0) {
		...
		(function(d, s) {
			...
			d.querySelectorAll = function(r, c, i, j, a) {
				a = d.all, c = [], r = r.replace(/\[for\b/gi, '[htmlFor')
						.split(',');
				for (i = r.length; i--;) {
					s.addRule(r[i], 'k:v');
					for (j = a.length; j--;)
...
<body>
	...
	<%
		...
		// 	}
	%>
	<html:form styleId="testForm" action="/login" focus="userName"
		method="POST">
		Username : 
...
<script type="text/javascript">
	...
	function maskInputDetail() {
		...
		for (var i = 0; i < elements.length; i++) {
			...
			var lastHalfText = label.innerText.slice(label.innerText.length
					- half);
			var beginHalfText = '*';
